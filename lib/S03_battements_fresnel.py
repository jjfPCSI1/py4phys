...
COLORS = ['r','g','b']  # Définition des couleurs pour chaque flèche
...
F2, =plt.plot(fleche_x,fleche_y,'b',lw=2)
...
# La flèche totale
Ft ,=plt.plot(2*fleche_x,2*fleche_y,'r',lw=2)
plt.axis('equal')                   # Aspect carré
plt.axis([-3.2,3.2,-2.2,2.2])       # Centrage de la figure
...
somme=np.cos((omega+delta_omega)*temps)+np.cos(omega*temps)
...
def animate(i):               # Fonction qui met à jour les données
    ...
    fx3,fy3=rotation_fleche((omega+0.5*delta_omega)*t)
    F1.set_xdata(fx)          # Mise à jour de la flèche 1
    F1.set_ydata(fy)
    F2.set_xdata(fx2)         # Mise à jour de la flcèeh 2
    F2.set_ydata(fy2)
    F21.set_xdata(fx2+fx[1])  # Premier parallèlogramme
    F21.set_ydata(fy2+fy[1])
    F12.set_xdata(fx+fx2[1])  # Second parallèlogramme
    F12.set_ydata(fy+fy2[1])
    Ft.set_xdata(2*np.cos(0.5*delta_omega*t)*fx3)
    Ft.set_ydata(2*np.cos(0.5*delta_omega*t)*fy3)
